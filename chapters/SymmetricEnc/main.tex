...
\section{PRNGs}
  ...
  \end{enumerate}
  %
  Recall we say that $D$ \emph{succeeds} when it outputs $1$. It should be clear that if $w$ is a truly random string, $D$ succeeds with probability $1/2$ (because \sgg{Enc} is exactly the OTP), whereas if $w$ is a pseudorandom string (output by $G$), $D$ succeeds with probability $1/2 + p(n)$:
  %
  \begin{equation}
